...
def main(lowres=False, usesaved=False, times=[22, 73]):
    ...
    basefilename = os.path.join(os.path.dirname(__file__), '..', 'data_out',
        r'VARIABLE_FB_PLOT_DATA_' + (r'LR' if lowres else r'HR'))
    ...
    if usesaved:
        ...
        x, t, E, T = WE.Integration(lowres, varyFB=True)
    ...
    a1.plot(t, xi_deg, color='b', linewidth=1.5,
        label=r'Variable $F_\mathrm{b}$: $%.1f < F_\mathrm{b}$ /Wm$^{-2}< %.1f$' % (
        params.FB_ICE, params.FB_ICE+params.DELTA_FB))
